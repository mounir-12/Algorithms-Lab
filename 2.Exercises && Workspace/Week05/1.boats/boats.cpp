...
bool operator<(Boat b1, Boat b2) { // operator< to determine which one is less prioritary
	...
	// if they have the same right_corner (this happens only if one or more boats were moved) then
	// we prioritize the one that was moved first since all boats that have the same right corner use the same space and
	// if only 1 boat was moved, then we know that it doesn't overlap anymore with the currently chosen boats 
...
void testcase() {
	...
	// we start by pushing all boats such that their right corner falls at the ring pos
	// i,e we try to keep as much space as possible to the right of the boat
	// some boats will be overlapping with others
	// we then start choosing boats, choosing first the one that keeps as much space as 
	// possible available to its right (i,e comsumes the least amount of available space to
	// right of already chosen boats)
	
	// We establish a priority system between boats such that the most prioritary is the one with the smallest
	// right_corner.
	// we keep track of "limit_corner" which is the right corner of the last chosen boat
	 
	for(int i=0; i<n; i++) {
		int l,p; std::cin >> l >> p;
		boats.push(Boat(l,p,p)); // all boats pushed to the left
	}
	...
	while(!boats.empty()) {
		Boat b = boats.top(); boats.pop(); // pick the most prioritary boat and remove it from the queue
		...
		if(left_corner >= limit_corner) { // boat doesn't overlap with previously chosen boats
			chosen_boats.push_back(b); // choose this boat
			...
			boats.push(b); // push it back to the queue to reorder it with the others depending on priority
		} 
	}
