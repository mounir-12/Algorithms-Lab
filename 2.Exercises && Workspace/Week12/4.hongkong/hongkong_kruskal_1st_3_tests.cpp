...
void dfs(Graph& G, Vertex s, WeightMap& weightmap, std::vector<bool>& visited, std::vector<EK::FT>& widthmap) {
	OutEdgeIt oebeg, oeend;
	// check all neighbors of u which one is its predecessor
	for(boost::tie(oebeg, oeend) = boost::out_edges(s, G); oebeg != oeend; ++oebeg) {
		Vertex v = boost::target(*oebeg, G);
		if(visited[v]) continue;
		visited[v] = true;
		widthmap[v] = CGAL::min(widthmap[s], weightmap[*oebeg]);
		dfs(G, v, weightmap, visited, widthmap);		
	}
}
...
void testcase() {	
	...
	V++;
	WeightMap weightmapG = boost::get(boost::edge_weight, G);
	...
	for(Face_iterator f = t.finite_faces_begin(); f != t.finite_faces_end(); ++f) { // loop over finite faces
		...
		for(int i=0; i<3; i++) {
			...
			add_edge(u, v, G, weightmapG, squared_width);
		}
	}
	...
	// build the MST
	Graph T(V);
	WeightMap weightmapT = boost::get(boost::edge_weight, T);
	
	for(Edge e: mst) {
		Vertex u = boost::source(e, G), v = boost::target(e, G);
		EK::FT weight = -weightmapG[e];
		add_edge(u, v, T, weightmapT, weight);
	}
	
	// perform dfs starting from the infinite vertex
	std::vector<EK::FT> widthmap(V);
	std::vector<bool> visited(V, false);
	widthmap[infinite] = DBL_MAX;
	visited[infinite] = true;
	dfs(T, infinite, weightmapT, visited, widthmap);
	...
	for(int i=0; i<m; i++) {
	 ...
	 while(disks_it != my_disks.end()) {
		 ...
		 int i = d.index; // index where to store the answer
		 ...
		 if(CGAL::squared_distance(p, q) >= d.squared_r) { // not touching any tree, we can move disk to circumcenter
			 ...
			 if(squared_circumradiuses[w] >= 4*d.squared_r) { // moving to the circumcenter is enough
				 answer = 'y'; goto endloop;
			 }
			 ...
			 if(squared_circumradiuses[set_w] >= 4*d.squared_r) { // the disk is in a component where it can move freely to some safe face
				 answer = 'y'; goto endloop;
			 }
			 ...
			 while(edges_it != my_edges.end() and edges_it->squared_width >= 4*d.squared_r) { // current disk can move along this edge, add it
				 ...
				 if(set_u != set_v) {
					 ...
					 squared_circumradiuses[set_u] = CGAL::max(squared_circumradiuses[set_u], squared_circumradiuses[set_v]);
					 squared_circumradiuses[set_v] = squared_circumradiuses[set_u];
				 }
				 edges_it++; // next edge
			 }
			 set_w = ds.find_set(w); // get the component again
			 if(squared_circumradiuses[set_w] >= 4*d.squared_r) { // do the check again after adding all edges along which the disk can move
				answer = 'y'; goto endloop;
			 }*/
